    ...
    try {
      ...
      if (parsed.windows && Array.isArray(parsed.windows)) {
        newState.windows = parsed.windows.map((w: any, idx: number) => ({
          ...
          timeRange: w.timeRange || `${idx * 3}s - ${(idx + 1) * 3}s`,
            ...
            <div className="space-y-1">
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <span>{translations.tabTimeline}</span>
                </div>
                <span className="px-1.5 py-0.2 bg-emerald-950 text-emerald-400 text-[9px] font-extrabold rounded border border-emerald-900/30">
                  {state.windows.length}x
                </span>
              </button>
