    ...
    <div className="space-y-6 pb-28 animate-fadeIn">
      ...
      <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-extrabold text-slate-900">
              {isEn ? 'Maestro Multi-Window Timeline Stitcher' : 'Maestro Multi-Window Timeline Stitcher'}
            </h2>
          </div>
          <p className="text-xs text-slate-500 mt-1 max-w-2xl font-medium">
            {isEn
              ? 'Connect multiple time windows into a continuous MiniMax H3 video clip. Maestro ensures smooth camera momentum and seamless consistency across window transitions.'
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <button
            ...
            className="flex-1 md:flex-initial flex items-center justify-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 rounded-xl text-xs font-extrabold transition-all shadow-xs cursor-pointer"
          >
            {copiedScript ? (
              <>
                <Check className="w-4 h-4 text-emerald-950" />
                {isEn ? 'Script Copied!' : 'Script Kopiert!'}
              </>
            ) : (
              <>
                <Copy className="w-4 h-4" />
                {isEn ? 'Copy Maestro Script' : 'Maestro Script Kopieren'}
              </>
            )}
          </button>
        </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 font-medium">
                      {idx === 0
                        ? (isEn ? 'Initial Opening Shot' : 'Anfangs-Sequenz (Initial Shot)')
                        : (isEn ? `Cut / Seamless transition from Window ${idx}` : `Schnitt / Nahtloser Übergang aus Window ${idx}`)}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    placeholder={isEn ? 'e.g. 0s - 3s' : 'z.B. 0s - 3s'}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1 md:col-span-2">
                  <label className="block text-xs font-bold text-slate-700">
                    {isEn ? 'Window Prompt (Scene action for this segment)' : 'Window Prompt (Szenen-Aktion für diesen Zeitabschnitt)'}
                  </label>
                  <textarea
                    value={win.prompt}
                    onChange={(e) =>
                    ...
                    placeholder={isEn ? `Action for Window ${win.windowNumber}...` : `Aktion für Window ${win.windowNumber}...`}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={isEn ? 'e.g. Slow forward tracking shot' : 'z.B. Slow forward tracking shot'}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">
                    {isEn ? 'Continuity Directive' : 'Übergangs-Logik (Continuity Directive)'}
                  </label>
                  <input
                    type="text"
                    ...
                    placeholder={isEn ? 'e.g. Seamless continuation from W1' : 'z.B. Nahtloser Kamera-Schwung fortführen aus W1'}
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-xs text-slate-900 focus:outline-none focus:border-amber-500 focus:bg-white"
                  />
                </div>
              </div>
            ...
            {idx < windows.length - 1 && (
              <div className="flex items-center justify-center py-1">
                <div className="flex items-center gap-2 text-xs text-amber-900 bg-amber-100 border border-amber-300 px-3.5 py-1 rounded-full font-mono font-bold shadow-xs">
                  ...
                  <span>
                    {isEn
                      ? `Camera & Momentum progression into Window ${idx + 2}`
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-2 text-slate-100 shadow-md">
        <div className="flex items-center justify-between text-xs font-mono text-slate-400">
          <span>{isEn ? 'GENERATED MAESTRO TIMELINE SCRIPT' : 'GENERATED MAESTRO TIMELINE CODE'}</span>
